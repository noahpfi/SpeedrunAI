...
public class ClientProfile extends GameProfile {
    ...
    public static String createRandomName() {
        ...
        for (int j = 0; j < i; j++) {
            name = name.concat(ChatUtils.ALPHABET[new Random().nextInt(ChatUtils.ALPHABET.length)]);
        }
    ...
    private static String[] getSkinFromMojang(String name) {
        try {
            String uuid = JsonParser.parseReader(new InputStreamReader(new URL("https://api.mojang.com/users/profiles/minecraft/" + name).openStream())).getAsJsonObject().get("id").getAsString();
            JsonObject property = JsonParser.parseReader(new InputStreamReader(new URL("https://sessionserver.mojang.com/session/minecraft/profile/" + uuid + "?unsigned=false").openStream())).getAsJsonObject().get("properties").getAsJsonArray().get(0).getAsJsonObject();
